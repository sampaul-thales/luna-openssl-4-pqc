...
static int ecx_import(void *keydata, int selection, const OSSL_PARAM params[])
{
    ...
    if (ok) {
        // NOTE: populate the public key properly for hsm,
        // considering the public key can be derived from private key
        if (key->privkey != NULL) {
            LUNA_PRINTF(("luna_prov_ecx_public_from_private\n"));
            ok = luna_prov_ecx_public_from_private(key);
        }
    }
                         ...
                         OSSL_PARAM params[], int include_private)
{
    ...
    LUNA_PRINTF(("ossl_param_build_set_octet_string\n"));
                          ...
                          const OSSL_PARAM params[], ECX_KEY_TYPE type)
{
    ...
    if ((gctx = OPENSSL_zalloc(sizeof(*gctx))) != NULL) {
        ...
        case ECX_KEY_TYPE_X25519:
            gctx->lunakeyctx = LUNA_OQS_malloc_from_ecxgen(gctx, "x25519");
            break;
        case ECX_KEY_TYPE_X448:
            gctx->lunakeyctx = LUNA_OQS_malloc_from_ecxgen(gctx, "x448");
            break;
        case ECX_KEY_TYPE_ED25519:
            gctx->lunakeyctx = LUNA_OQS_malloc_from_ecxgen(gctx, "ed25519");
            break;
        case ECX_KEY_TYPE_ED448:
            gctx->lunakeyctx = LUNA_OQS_malloc_from_ecxgen(gctx, "ed448");
            break;
        }

        if (!ecx_gen_set_params(gctx, params)) {
            LUNA_OQS_free(gctx->lunakeyctx);
            gctx->lunakeyctx = 0;
            OPENSSL_free(gctx);
            gctx = NULL;
        }
...
static void *ecx_gen(struct ecx_gen_ctx *gctx)
{
    ...
    if (gctx->dhkem_ikm != NULL && gctx->dhkem_ikmlen != 0) {
        if (gctx->type == ECX_KEY_TYPE_ED25519
            ...
            goto err;
        LUNA_PRINTF(("ossl_ecx_dhkem_derive_private\n"));
        /* was: if (!ossl_ecx_dhkem_derive_private(key, privkey, gctx->dhkem_ikm, gctx->dhkem_ikmlen)) */
        if (!luna_prov_ecx_dhkem_derive_private(gctx, key, privkey))
            goto err;
    } else
#endif
    {
...
static int luna_ecx_key_pairwise_check(const ECX_KEY *ecx, int type);
...
static int ecx_validate(const void *keydata, int selection, int type, size_t keylen)
{
    ...
    if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) == OSSL_KEYMGMT_SELECT_KEYPAIR)
        ok = ok && luna_ecx_key_pairwise_check(ecx, type);
...
#define MAKE_KEYMGMT_FUNCTIONS(alg) \
    const OSSL_DISPATCH luna_##alg##_keymgmt_functions[] = { \
...
static int luna_ecx_key_pairwise_check(const ECX_KEY *ecx, int type)
{
    ...
    if (luna_prov_check_is_software(rc_check)) {
        return ecx_key_pairwise_check(ecx, type);
