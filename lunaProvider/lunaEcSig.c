                                 ...
                                 const OSSL_PARAM params[], int operation)
{
    ...
    if (ec != NULL) {
        if (!luna_prov_ec_check_key(ctx->libctx, ec, operation == EVP_PKEY_OP_SIGN))
            return 0;
        if (!EC_KEY_up_ref(ec))
            return 0;
        EC_KEY_free(ctx->ec);
                        ...
                        const unsigned char *tbs, size_t tbslen)
{
    ...
    if (!luna_prov_is_running() || (ctx->mdsize != 0 && tbslen != ctx->mdsize))
        return 0;

    LUNA_PRINTF(("ECDSA_verify\n"));
    if (siglen > LUNA_PROV_MAX_BUFFER || tbslen > LUNA_PROV_MAX_BUFFER)
        return 0;
    return luna_prov_ECDSA_verify(0, tbs, tbslen, sig, siglen, ctx->ec);
}
                          ...
                          const char *mdprops)
{
    ...
    if (mdprops == NULL)
        mdprops = ctx->propq;
    md = LUNAPROV_EVP_MD_fetch(ctx->libctx, mdname, mdprops);
    if (md == NULL) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_DIGEST,
    ...
    md_nid = LUNAPROV_ossl_digest_get_approved_nid_with_sha1(ctx->libctx, md,
                                                    sha1_allowed);
    if (md_nid < 0) {
    ...
    ctx->mdctx = NULL;
    ctx->md = md;
    ctx->mdsize = LUNAPROV_EVP_MD_get_size(ctx->md);
                                        ...
                                        int operation)
{
    ...
    if (ctx->mdctx == NULL) {
        ctx->mdctx = LUNAPROV_EVP_MD_CTX_new();
        if (ctx->mdctx == NULL)
            goto error;
    }

    LUNA_PRINTF(("EVP_DigestInit_ex2\n"));
    if (!LUNAPROV_EVP_DigestInit_ex2(ctx->mdctx, ctx->md, params))
        goto error;
    LUNA_PRINTF(("EVP_DigestInit_ex2 ok\n"));
    return 1;
error:
    LUNAPROV_EVP_MD_CTX_free(ctx->mdctx);
    ctx->mdctx = NULL;
                                   ...
                                   size_t datalen)
{
    ...
    return LUNAPROV_EVP_DigestUpdate(ctx->mdctx, data, datalen);
}

static int ecdsa_digest_sign_final(void *vctx, unsigned char *sig, size_t *siglen,
                            size_t sigsize)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
    ...
    if (!luna_prov_is_running() || ctx == NULL || ctx->mdctx == NULL)
        return 0;
...
static int ecdsa_digest_verify_final(void *vctx, const unsigned char *sig,
                              size_t siglen)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
    unsigned char digest[EVP_MAX_MD_SIZE];
...
static int ecdsa_get_ctx_params(void *vctx, OSSL_PARAM *params)
{
    ...
    if (p != NULL && !OSSL_PARAM_set_utf8_string(p, ctx->md == NULL
                                                    ? ctx->mdname
                                                    : LUNAPROV_EVP_MD_get0_name(ctx->md)))
        return 0;
...
static int ecdsa_get_ctx_md_params(void *vctx, OSSL_PARAM *params)
{
    ...
    return LUNAPROV_EVP_MD_CTX_get_params(ctx->mdctx, params);
}
...
static const OSSL_PARAM *ecdsa_gettable_ctx_md_params(void *vctx)
{
    ...
    return LUNAPROV_EVP_MD_gettable_ctx_params(ctx->md);
}
...
static int ecdsa_set_ctx_md_params(void *vctx, const OSSL_PARAM params[])
{
    ...
    return LUNAPROV_EVP_MD_CTX_set_params(ctx->mdctx, params);
}
...
static const OSSL_PARAM *ecdsa_settable_ctx_md_params(void *vctx)
{
    ...
    return LUNAPROV_EVP_MD_settable_ctx_params(ctx->md);
}

const OSSL_DISPATCH luna_ec_signature_functions[] = {
