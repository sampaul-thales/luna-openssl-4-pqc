                    ...
                    int operation)
{
    ...
    if (!luna_prov_is_running() || prsactx == NULL || vrsa == NULL)
        return 0;
                       ...
                       size_t outsize, const unsigned char *in, size_t inlen)
{
    ...
    if (prsactx->pad_mode == RSA_PKCS1_OAEP_PADDING) {
        ...
        if (prsactx->oaep_md == NULL) {
            OPENSSL_free(tbuf);
            prsactx->oaep_md = LUNAPROV_EVP_MD_fetch(prsactx->libctx, "SHA-1", NULL);
            ERR_raise(ERR_LIB_PROV, ERR_R_INTERNAL_ERROR);
            return 0;
        }
...
#if 1
        {
            const EVP_MD *tmp_oaep_md = LUNAPROV_EVP_MD_get_tmp_md(prsactx->oaep_md);
            const EVP_MD *tmp_mgf1_md = LUNAPROV_EVP_MD_get_tmp_md(prsactx->mgf1_md);
            if ( (tmp_oaep_md != NULL) && (tmp_mgf1_md != NULL) ) {
        ...
        LUNA_PRINTF(("RSA_public_encrypt@1a\n"));
        ...
        LUNA_PRINTF(("RSA_public_encrypt@2\n"));
        ret = RSA_public_encrypt(inlen, in, out, prsactx->rsa,
                                 prsactx->pad_mode);
    }

    /* a return value of zero (from RSA_public_encrypt) is not an error */
    if (ret < 0)
        return ret;
    *outlen = ret;
    return 1;
                       ...
                       size_t outsize, const unsigned char *in, size_t inlen)
{
    ...
    if (prsactx->pad_mode == RSA_PKCS1_WITH_TLS_PADDING) {
        ...
        if (out == NULL) {
            *outlen = (size_t)rsasize;
            return 1;
        }

        if (outsize < (size_t)rsasize) {
            ERR_raise(ERR_LIB_PROV, PROV_R_BAD_LENGTH);
            return 0;
        }
    }

    size_t len = (size_t)rsasize;

    /* coverity: check for overflowed value */
    if (inlen < 1 || inlen > LUNA_PROV_MAX_BUFFER)
        return 0;
    ...
    if (prsactx->pad_mode == RSA_PKCS1_OAEP_PADDING
        ...
        if (luna_prov_check_is_software(rc_check)) {
            LUNA_PRINTF(("RSA_private_decrypt: software: pad_mode = %d\n", prsactx->pad_mode));
            ret = luna_prov_RSA_private_decrypt(inlen, in, tbuf, prsactx->rsa, RSA_NO_PADDING);
            /*
            ...
            } else if (prsactx->pad_mode == RSA_PKCS1_WITH_TLS_PADDING) {
            ...
            if (prsactx->pad_mode == RSA_PKCS1_OAEP_PADDING) {
                ...
                if ( (tmp_oaep_md != NULL) && (tmp_mgf1_md != NULL) ) {
                    ...
                    oaep_params.oaep_md = tmp_oaep_md;
                    oaep_params.mgf1_md = tmp_mgf1_md;
                    oaep_params.oaep_label = prsactx->oaep_label;
                    oaep_params.labellen = prsactx->oaep_labellen;
                    ret = luna_prov_rsa_priv_dec_x509(&oaep_params, inlen, in,
                            (size_t)rsasize, out, prsactx->rsa, RSA_PKCS1_OAEP_PADDING);
                } else {
                    ret = 0;
                }
            } else if (prsactx->pad_mode == RSA_PKCS1_WITH_TLS_PADDING) {
                LUNA_PRINTF(("RSA_private_decrypt: hardware: pad_mode = RSA_PKCS1_WITH_TLS_PADDING\n"));
                ret = luna_prov_RSA_private_decrypt(inlen, in, tbuf, prsactx->rsa, RSA_NO_PADDING);
                /*
                 * With no padding then, on success ret should be len, otherwise an
                 * error occurred (non-constant time)
        ...
        LUNA_PRINTF(("RSA_private_decrypt: pad_mode = %d\n", prsactx->pad_mode));
        ret = luna_prov_RSA_private_decrypt(inlen, in, out, prsactx->rsa, prsactx->pad_mode);
    }

    /* coverity: check for negative returns */
    if (ret < 0) {
        return 0;
    }

    /* coverity: *outlen = constant_time_select_s(constant_time_msb_s(ret), *outlen, ret); */
    *outlen = ret;
    /* coverity: ret = constant_time_select_int(constant_time_msb(ret), 0, 1); */
    return 1;
}
...
static void rsa_freectx(void *vprsactx)
{
    ...
    LUNAPROV_EVP_MD_free(prsactx->oaep_md);
...
static void *rsa_dupctx(void *vprsactx)
{
    ...
    *dstctx = *srcctx;
    ...
    if (dstctx->oaep_md != NULL && !LUNAPROV_EVP_MD_up_ref(dstctx->oaep_md)) {
        RSA_free(dstctx->rsa);
        OPENSSL_free(dstctx);
        return NULL;
    }

    if (dstctx->mgf1_md != NULL && !LUNAPROV_EVP_MD_up_ref(dstctx->mgf1_md)) {
        RSA_free(dstctx->rsa);
        LUNAPROV_EVP_MD_free(dstctx->oaep_md);
        OPENSSL_free(dstctx);
        return NULL;
    }
...
static int rsa_get_ctx_params(void *vprsactx, OSSL_PARAM *params)
{
    ...
    if (p != NULL && !OSSL_PARAM_set_utf8_string(p, prsactx->oaep_md == NULL
                                                    ? ""
                                                    : LUNAPROV_EVP_MD_get0_name(prsactx->oaep_md)))
        return 0;

    p = OSSL_PARAM_locate(params, OSSL_ASYM_CIPHER_PARAM_MGF1_DIGEST);
    if (p != NULL) {
        LUNAPROV_EVP_MD *mgf1_md = prsactx->mgf1_md == NULL ? prsactx->oaep_md
                                                   : prsactx->mgf1_md;

        if (!OSSL_PARAM_set_utf8_string(p, mgf1_md == NULL
                                           ? ""
                                           : LUNAPROV_EVP_MD_get0_name(mgf1_md)))
        return 0;
    }
...
static int rsa_set_ctx_params(void *vprsactx, const OSSL_PARAM params[])
{
    ...
    if (p != NULL) {
        ...
        LUNAPROV_EVP_MD_free(prsactx->oaep_md);
        prsactx->oaep_md = LUNAPROV_EVP_MD_fetch(prsactx->libctx, mdname, mdprops);
    ...
    if (p != NULL) {
        ...
        if (pad_mode == RSA_PKCS1_OAEP_PADDING && prsactx->oaep_md == NULL) {
            prsactx->oaep_md = LUNAPROV_EVP_MD_fetch(prsactx->libctx, "SHA1", mdprops);
            if (prsactx->oaep_md == NULL)
                return 0;
        }
    ...
    if (p != NULL) {
        ...
        LUNAPROV_EVP_MD_free(prsactx->mgf1_md);
        prsactx->mgf1_md = LUNAPROV_EVP_MD_fetch(prsactx->libctx, mdname, str);
...
const OSSL_DISPATCH luna_rsa_asym_cipher_functions[] = {
