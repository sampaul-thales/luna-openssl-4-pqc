...
static OSSL_FUNC_keymgmt_validate_fn luna_ec_validate;
                  ...
                  unsigned char **pub_key)
{
    ...
    if (pub_point != NULL) {
        ...
        if (p != NULL || tmpl != NULL) {
            ...
            /* NOTE: was: point_conversion_form_t format = POINT_CONVERSION_COMPRESSED; */
 ...
 */
#include "implementations/keymgmt/ec_kmgmt_imexport.inc"
...
static int luna_ec_key_public_check_quick(const EC_KEY *eckey, BN_CTX *ctx);
static int luna_ec_key_public_check(const EC_KEY *eckey, BN_CTX *ctx);
static int luna_ec_key_private_check(const EC_KEY *eckey);
static int luna_ec_key_pairwise_check(const EC_KEY *eckey, BN_CTX *ctx);

static
int luna_ec_validate(const void *keydata, int selection, int checktype)
{
    const EC_KEY *eck = keydata;
    int ok = 1;
    BN_CTX *ctx = NULL;
    ...
    if ((selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) != 0) {
        if (checktype == OSSL_KEYMGMT_VALIDATE_QUICK_CHECK)
            ok = ok && luna_ec_key_public_check_quick(eck, ctx);
        else
            ok = ok && luna_ec_key_public_check(eck, ctx);
    }

    if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0)
        ok = ok && luna_ec_key_private_check(eck);

    if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) == OSSL_KEYMGMT_SELECT_KEYPAIR)
        ok = ok && luna_ec_key_pairwise_check(eck, ctx);
                         ...
                         const OSSL_PARAM params[])
{
    ...
    if (!luna_prov_is_running() || (selection & (EC_POSSIBLE_SELECTIONS)) == 0)
        return NULL;
...
static int ec_gen_set_template(void *genctx, void *templ)
{
    ...
    if (!luna_prov_is_running() || gctx == NULL || ec == NULL)
        return 0;
...
static int ec_gen_set_params(void *genctx, const OSSL_PARAM params[])
{
    ...
    /* override CKA_TOKEN, etc */
    COPY_INT_PARAM(params, LUNA_PROV_PKEY_PARAM_FLAGS, gctx->lunaflags);
...
err:
    EC_GROUP_free(group);
    FREE_UTF8_PARAM(gctx->group_name);
    FREE_UTF8_PARAM(gctx->field_type);
    FREE_UTF8_PARAM(gctx->encoding);
    FREE_UTF8_PARAM(gctx->pt_format);
    FREE_UTF8_PARAM(gctx->group_check);
    FREE_BN_PARAM(gctx->p);
    FREE_BN_PARAM(gctx->a);
    FREE_BN_PARAM(gctx->b);
    FREE_BN_PARAM(gctx->order);
    FREE_BN_PARAM(gctx->cofactor);
    FREE_OCTET_PARAM(gctx->seed, gctx->seed_len);
    FREE_OCTET_PARAM(gctx->gen, gctx->gen_len);
    return 0;
}
                                                ...
                                                ossl_unused void *provctx)
{
    static OSSL_PARAM settable[] = {
        ...
        OSSL_PARAM_octet_string(OSSL_PKEY_PARAM_EC_SEED, NULL, 0),
        /* override CKA_TOKEN, etc */
        OSSL_PARAM_int(LUNA_PROV_PKEY_PARAM_FLAGS, NULL),
        OSSL_PARAM_END
    };
                         ...
                         int sm2_wanted)
{
    ...
    if (luna_prov_is_running() && reference_sz == sizeof(ec)) {
...
const OSSL_DISPATCH luna_ec_keymgmt_functions[] = {
    ...
    { OSSL_FUNC_KEYMGMT_VALIDATE, (void (*)(void))luna_ec_validate },
...
#ifndef FIPS_MODULE
# ifndef OPENSSL_NO_SM2
const OSSL_DISPATCH luna_sm2_keymgmt_functions[] = {
...
static int luna_ec_key_public_check_quick(const EC_KEY *eckey, BN_CTX *ctx)
{
    ...
    if (luna_prov_check_is_software(rc_check)) {
        return ossl_ec_key_public_check_quick(eckey, ctx);
...
static int luna_ec_key_public_check(const EC_KEY *eckey, BN_CTX *ctx)
{
    ...
    const int rc_check = luna_prov_ec_check_public(eckey);
    if (luna_prov_check_is_software(rc_check)) {
        return ossl_ec_key_public_check(eckey, ctx);
    } else if (luna_prov_check_is_hardware(rc_check)) {
        return 1;
    } else {
        return 0;
...
static int luna_ec_key_private_check(const EC_KEY *eckey)
{
    ...
    if (luna_prov_check_is_software(rc_check)) {
        return ossl_ec_key_private_check(eckey);
    } else if (luna_prov_check_is_hardware(rc_check)) {
        return 1;
    } else {
        return 0;
...
static int luna_ec_key_pairwise_check(const EC_KEY *eckey, BN_CTX *ctx)
{
    ...
    const int rc_check = luna_prov_ec_check_private(eckey);
    if (luna_prov_check_is_software(rc_check)) {
        return ossl_ec_key_pairwise_check(eckey, ctx);
    } else if (luna_prov_check_is_hardware(rc_check)) {
        return 1;
    } else {
        return 0;
