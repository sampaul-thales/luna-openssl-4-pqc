...
{
    if (prsactx->md != NULL)
        return LUNAPROV_EVP_MD_get_size(prsactx->md);
    return 0;
}
    ...
    switch(prsactx->pad_mode) {
        ...
        case RSA_PKCS1_PSS_PADDING:
            if (rsa_pss_restricted(prsactx))
                if ((mdname != NULL && !LUNAPROV_EVP_MD_is_a(prsactx->md, mdname))
                    || (mgf1_mdname != NULL
                        && !LUNAPROV_EVP_MD_is_a(prsactx->mgf1_md, mgf1_mdname))) {
...
static int rsa_check_parameters(PROV_RSA_CTX *prsactx, int min_saltlen)
{
    if (prsactx->pad_mode == RSA_PKCS1_PSS_PADDING) {
        ...
        max_saltlen = RSA_size(prsactx->rsa) - LUNAPROV_EVP_MD_get_size(prsactx->md);
...
static int rsa_pss_compute_saltlen(PROV_RSA_CTX *ctx)
{
    ...
    if (saltlen == RSA_PSS_SALTLEN_DIGEST) {
        saltlen = LUNAPROV_EVP_MD_get_size(ctx->md);
    } else if (saltlen == RSA_PSS_SALTLEN_AUTO || saltlen == RSA_PSS_SALTLEN_MAX) {
        saltlen = RSA_size(ctx->rsa) - LUNAPROV_EVP_MD_get_size(ctx->md) - 2;
                        ...
                        const char *mdprops)
{
    ...
    if (mdname != NULL) {
        ...
        int md_nid = LUNAPROV_ossl_digest_rsa_sign_get_md_nid(ctx->libctx, md,
                                                     sha1_allowed);
        ...
        if (!ctx->mgf1_md_set) {
            if (!LUNAPROV_EVP_MD_up_ref(md)) {
                LUNAPROV_EVP_MD_free(md);
                return 0;
            }
            LUNAPROV_EVP_MD_free(ctx->mgf1_md);
            ctx->mgf1_md = md;
...
{
    size_t len;
    LUNAPROV_EVP_MD *md = NULL;
    int mdnid;

    if (mdprops == NULL)
        mdprops = ctx->propq;

    if ((md = LUNAPROV_EVP_MD_fetch(ctx->libctx, mdname, mdprops)) == NULL) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_DIGEST,
                       "%s could not be fetched", mdname);
        return 0;
    }
    /* The default for mgf1 is SHA1 - so allow SHA1 */
    if ((mdnid = LUNAPROV_ossl_digest_rsa_sign_get_md_nid(ctx->libctx, md, 1)) <= 0
                               ...
                               const OSSL_PARAM params[], int operation)
{
    ...
    if (!luna_prov_is_running() || prsactx == NULL)
        return 0;
    ...
    if (vrsa != NULL) {
        if (!luna_prov_rsa_check_key(prsactx->libctx, vrsa, operation))
            return 0;
...
typedef struct luna_pss_params {
    const EVP_MD *sig_md;
                    ...
                    size_t sigsize, const unsigned char *tbs, size_t tbslen)
{
    ...
    if (mdsize != 0) {
        ...
        /* no worries here about unsupported digests (MD5, MDC, etc) */

        switch (prsactx->pad_mode) {
        case RSA_X931_PADDING:
            if ((size_t)rsasize < tbslen + 1) {
            ...
            LUNA_PRINTF(("RSA_private_encrypt@1\n"));
            ret = luna_prov_RSA_private_encrypt(tbslen + 1, prsactx->tbuf, sig, prsactx->rsa, RSA_X931_PADDING);
            clean_tbuf(prsactx);
            break;
        ...
        case RSA_PKCS1_PADDING:
            {
                unsigned int sltmp;
                LUNA_PRINTF(("RSA_sign\n"));
                ret = luna_prov_RSA_sign(prsactx->mdnid, tbs, tbslen, sig, &sltmp, prsactx->rsa);
                if (ret <= 0) {
        ...
        case RSA_PKCS1_PSS_PADDING:
            ...
            rc_check = luna_prov_rsa_check_private(prsactx->rsa);
            if (luna_prov_check_is_software(rc_check))
            {
                const EVP_MD *tmp_md = LUNAPROV_EVP_MD_get_tmp_md(prsactx->md);
                const EVP_MD *tmp_mgf1_md = LUNAPROV_EVP_MD_get_tmp_md(prsactx->mgf1_md);
                if ( (tmp_md != NULL) && (tmp_mgf1_md != NULL) ) {
                ...
                if (ret == 1) {
                    LUNA_PRINTF(("RSA_private_encrypt@2\n"));
                    ret = luna_prov_RSA_private_encrypt(rsasize, prsactx->tbuf, sig, prsactx->rsa, RSA_NO_PADDING);
                }
            }
            else if (luna_prov_check_is_hardware(rc_check))
            {
                ...
                if ( (tmp_md != NULL) && (tmp_mgf1_md != NULL) ) {
                    ...
                    pss_params.sig_md = tmp_md;
                    pss_params.mgf1_md = tmp_mgf1_md;
                    pss_params.saltlen = prsactx->saltlen;
                    LUNA_PRINTF(("rsa_priv_enc_pkcs\n"));
                    ret = luna_prov_rsa_priv_enc_pkcs(&pss_params,
                            mdsize, tbs,
                            (size_t)rsasize, sig,
                            prsactx->rsa, RSA_PKCS1_PSS_PADDING);
                } else {
                    ret = 0;
                }
            }
        ...
        LUNA_PRINTF(("RSA_private_encrypt@3\n"));
        ret = luna_prov_RSA_private_encrypt(tbslen, tbs, sig, prsactx->rsa, prsactx->pad_mode);
    }
                              ...
                              size_t siglen)
{
    ...
    /* coverity: check for overflowed value */
    if (siglen > LUNA_PROV_MAX_BUFFER) {
        return 0;
    }
    ...
    if (prsactx->md != NULL) {
        ...
        case RSA_X931_PADDING:
            if (!setup_tbuf(prsactx))
                return 0;
            LUNA_PRINTF(("RSA_public_decrypt\n"));
            ...
            if (ret != LUNAPROV_EVP_MD_get_size(prsactx->md)) {
                ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_DIGEST_LENGTH,
                               "Should be %d, but got %d",
                               LUNAPROV_EVP_MD_get_size(prsactx->md), ret);
                return 0;
            }
                      ...
                      const unsigned char *tbs, size_t tbslen)
{
    ...
    if (prsactx->md != NULL) {
        switch (prsactx->pad_mode) {
        case RSA_PKCS1_PADDING:
            LUNA_PRINTF(("RSA_verify\n"));
            if (!luna_prov_RSA_verify(prsactx->mdnid, tbs, tbslen, sig, siglen, prsactx->rsa)) {
                ERR_raise(ERR_LIB_PROV, ERR_R_RSA_LIB);
                return 0;
            }
            return 1;
        case RSA_X931_PADDING:
            if (!setup_tbuf(prsactx))
                return 0;
            LUNA_PRINTF(("rsa_verify_recover\n"));
        ...
        case RSA_PKCS1_PSS_PADDING:
            {
                ...
                if (!setup_tbuf(prsactx))
                    return 0;
#if 1
                LUNA_PRINTF(("RSA_public_decrypt@1a\n"));
...
#else
                /* TODO: cannot use hsm for public key crypto here .. but that is ok */
                ret = -1;
#endif
        ...
        if (!setup_tbuf(prsactx))
            return 0;
        LUNA_PRINTF(("RSA_public_decrypt@2\n"));
                                      ...
                                      int operation)
{
    ...
    if (prsactx->mdctx == NULL) {
        prsactx->mdctx = LUNAPROV_EVP_MD_CTX_new();
        if (prsactx->mdctx == NULL)
            goto error;
    }

    LUNA_PRINTF(("EVP_DigestInit_ex2\n"));
    if (!LUNAPROV_EVP_DigestInit_ex2(prsactx->mdctx, prsactx->md, params))
        goto error;

    return 1;

error:
    LUNAPROV_EVP_MD_CTX_free(prsactx->mdctx);
    prsactx->mdctx = NULL;
    return 0;
}
                                        ...
                                        size_t datalen)
{
    ...
    return LUNAPROV_EVP_DigestUpdate(prsactx->mdctx, data, datalen);
}
                                 ...
                                 size_t *siglen, size_t sigsize)
{
    ...
    if (sig != NULL) {
        ...
        if (!LUNAPROV_EVP_DigestFinal_ex(prsactx->mdctx, digest, &dlen))
            return 0;
    }
                                  ...
                                  void *vrsa, const OSSL_PARAM params[])
{
    if (!luna_prov_is_running())
        return 0;
    LUNA_PRINTF(("rsa_digest_signverify_init\n"));
    return rsa_digest_signverify_init(vprsactx, mdname, vrsa,
                                      params, EVP_PKEY_OP_VERIFY);
}

static int rsa_digest_verify_final(void *vprsactx, const unsigned char *sig,
                            size_t siglen)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
    unsigned char digest[EVP_MAX_MD_SIZE];
...
static void rsa_freectx(void *vprsactx)
{
    ...
    LUNAPROV_EVP_MD_CTX_free(prsactx->mdctx);
    LUNAPROV_EVP_MD_free(prsactx->md);
    LUNAPROV_EVP_MD_free(prsactx->mgf1_md);
...
static void *rsa_dupctx(void *vprsactx)
{
    ...
    *dstctx = *srcctx;
    ...
    if (srcctx->mgf1_md != NULL && !LUNAPROV_EVP_MD_up_ref(srcctx->mgf1_md))
        goto err;
    ...
    if (p != NULL) {
        ...
        if (rsa_pss_restricted(prsactx)) {
            ...
            case RSA_PSS_SALTLEN_AUTO:
                ...
                break;
            case RSA_PSS_SALTLEN_DIGEST:
                if (prsactx->min_saltlen > LUNAPROV_EVP_MD_get_size(prsactx->md)) {
                    ERR_raise_data(ERR_LIB_PROV,
                                   PROV_R_PSS_SALTLEN_TOO_SMALL,
                                   "Should be more than %d, but would be "
                                   "set to match digest size (%d)",
                                   prsactx->min_saltlen,
                                   LUNAPROV_EVP_MD_get_size(prsactx->md));
                    return 0;
                }
                break;
            default:
...
static int rsa_get_ctx_md_params(void *vprsactx, OSSL_PARAM *params)
{
    ...
    return LUNAPROV_EVP_MD_CTX_get_params(prsactx->mdctx, params);
}
...
static const OSSL_PARAM *rsa_gettable_ctx_md_params(void *vprsactx)
{
    ...
    return LUNAPROV_EVP_MD_gettable_ctx_params(prsactx->md);
}
...
static int rsa_set_ctx_md_params(void *vprsactx, const OSSL_PARAM params[])
{
    ...
    return LUNAPROV_EVP_MD_CTX_set_params(prsactx->mdctx, params);
}
...
static const OSSL_PARAM *rsa_settable_ctx_md_params(void *vprsactx)
{
    ...
    return LUNAPROV_EVP_MD_settable_ctx_params(prsactx->md);
}

const OSSL_DISPATCH luna_rsa_signature_functions[] = {
