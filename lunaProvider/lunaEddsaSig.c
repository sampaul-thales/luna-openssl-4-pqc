...
#ifdef S390X_EC_ASM
#error "LUNA: unsupported S390X_EC_ASM"
...
static void *eddsa_newctx(void *provctx, const char *propq_unused)
{
    ...
    peddsactx->lunakeyctx = LUNA_OQS_malloc_from_eddsa(peddsactx, "edINVALID");
    if (peddsactx->lunakeyctx == NULL)
        return NULL;
                                        ...
                                        const OSSL_PARAM params[])
{
    ...
    if (edkey->type == ECX_KEY_TYPE_ED25519) {
        LUNA_OQS_refresh_alg_name(peddsactx->lunakeyctx, "ed25519");
    } else if (edkey->type == ECX_KEY_TYPE_ED448) {
        LUNA_OQS_refresh_alg_name(peddsactx->lunakeyctx, "ed448");
    }
                        ...
                        const unsigned char *tbs, size_t tbslen)
{
    ...
    if (S390X_CAN_SIGN(ED25519)
            ...
            && peddsactx->context_string_len == 0) {
        LUNA_PRINTF(("s390x_ed25519_digestsign\n"));
    ...
    int rc_tmp = 0;
    int rc_check = luna_prov_ecx_check_private(edkey);
    if (luna_prov_check_is_software(rc_check)) {
        LUNA_PRINTF(("ossl_ed25519_sign\n"));
        rc_tmp = ossl_ed25519_sign(sigret, tbs, tbslen, edkey->pubkey, edkey->privkey,
        ...
        LUNA_PRINTF(("luna_prov_ed25519_sign\n"));
        rc_tmp = luna_prov_ed25519_sign(peddsactx->lunakeyctx,
                sigret, siglen,
                tbs, tbslen);
    } else {
        rc_tmp = 0;
                      ...
                      const unsigned char *tbs, size_t tbslen)
{
    ...
    if (S390X_CAN_SIGN(ED448)
            && peddsactx->context_string_len == 0
            && peddsactx->prehash_flag == 0) {
        LUNA_PRINTF(("s390x_ed448_digestsign\n"));
    ...
    int rc_check = luna_prov_ecx_check_private(edkey);
    if (luna_prov_check_is_software(rc_check)) {
        LUNA_PRINTF(("ossl_ed448_sign\n"));
        rc_tmp = ossl_ed448_sign(peddsactx->libctx, sigret, tbs, tbslen,
                            edkey->pubkey, edkey->privkey,
                            peddsactx->context_string, peddsactx->context_string_len,
                            peddsactx->prehash_flag, edkey->propq);
    } else if (luna_prov_check_is_hardware(rc_check)) {
        LUNA_PRINTF(("luna_prov_ed448_sign\n"));
        rc_tmp = luna_prov_ed448_sign(peddsactx->lunakeyctx,
                sigret, siglen,
                tbs, tbslen);
    } else {
        rc_tmp = 0;
                          ...
                          size_t tbslen)
{
    ...
    if (S390X_CAN_SIGN(ED25519)
            ...
            && !peddsactx->context_string_flag
            && peddsactx->context_string_len == 0) {
        LUNA_PRINTF(("s390x_ed25519_digestverify\n"));
    ...
    LUNA_PRINTF(("ossl_ed25519_verify\n"));
                        ...
                        size_t tbslen)
{
    ...
    if (S390X_CAN_SIGN(ED448)
            && peddsactx->context_string_len == 0
            && peddsactx->prehash_flag == 0) {
        LUNA_PRINTF(("s390x_ed448_digestverify\n"));
...
static void eddsa_freectx(void *vpeddsactx)
{
    ...
    LUNA_OQS_free(peddsactx->lunakeyctx);
...
static void *eddsa_dupctx(void *vpeddsactx)
{
    ...
    *dstctx = *srcctx;
    ...
    dstctx->lunakeyctx = LUNA_OQS_malloc_from_eddsa_2(dstctx, srcctx->lunakeyctx); /* TODO: copied properly? */
...
const OSSL_DISPATCH luna_ed25519_signature_functions[] = {
...
const OSSL_DISPATCH luna_ed448_signature_functions[] = {
    { OSSL_FUNC_SIGNATURE_NEWCTX, (void (*)(void))eddsa_newctx },
    { OSSL_FUNC_SIGNATURE_DIGEST_SIGN_INIT,
      (void (*)(void))eddsa_digest_signverify_init },
    { OSSL_FUNC_SIGNATURE_DIGEST_SIGN,
