...
static OSSL_FUNC_keymgmt_validate_fn luna_rsa_validate;
...
static int rsa_has(const void *keydata, int selection)
{
    ...
    if (rsa == NULL || !luna_prov_is_running())
        return 0;
...
static int rsa_get_params(void *key, OSSL_PARAM params[])
{
    ...
    const int include_private = 1; /* TODO: is this configurable? */
    ...
    return (rsa_type != RSA_FLAG_TYPE_RSASSAPSS
        ...
        && ossl_rsa_todata(rsa, NULL, params, include_private);
}
...
static int luna_rsa_validate_pairwise(const RSA *key);
static int luna_rsa_validate_private(const RSA *key);
static int luna_rsa_validate_public(const RSA *key);

static int luna_rsa_validate(const void *keydata, int selection, int checktype)
{
    const RSA *rsa = keydata;
    int ok = 1;
    ...
    if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR)
        ...
        ok = ok && luna_rsa_validate_pairwise(rsa);
    } else {
        if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0)
            ok = ok && luna_rsa_validate_private(rsa);
        if ((selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) != 0)
            ok = ok && luna_rsa_validate_public(rsa);
    }
    return ok;
}
                         ...
                         int expected_rsa_type)
{
    ...
    if (luna_prov_is_running() && reference_sz == sizeof(rsa)) {
...
const OSSL_DISPATCH luna_rsa_keymgmt_functions[] = {
    ...
    { OSSL_FUNC_KEYMGMT_VALIDATE, (void (*)(void))luna_rsa_validate },
...
const OSSL_DISPATCH luna_rsapss_keymgmt_functions[] = {
...
{
    LUNA_PRINTF(("\n"));
    const int rc_check = luna_prov_rsa_check_private(key);
    if (luna_prov_check_is_software(rc_check)) {
        return ossl_rsa_validate_pairwise(key);
...
static int luna_rsa_validate_private(const RSA *key)
{
    ...
    const int rc_check = luna_prov_rsa_check_private(key);
    if (luna_prov_check_is_software(rc_check)) {
        return ossl_rsa_validate_private(key);
    } else if (luna_prov_check_is_hardware(rc_check)) {
        return 1;
    } else {
        return 0;
...
{
    LUNA_PRINTF(("\n"));
    const int rc_check = luna_prov_rsa_check_public(key);
    if (luna_prov_check_is_software(rc_check)) {
