...
static OSSL_FUNC_keyexch_init_fn luna_ecdh_init;
static OSSL_FUNC_keyexch_set_peer_fn luna_ecdh_set_peer;
static OSSL_FUNC_keyexch_derive_fn luna_ecdh_derive;
static OSSL_FUNC_keyexch_freectx_fn luna_ecdh_freectx;
static OSSL_FUNC_keyexch_dupctx_fn luna_ecdh_dupctx;
static OSSL_FUNC_keyexch_set_ctx_params_fn luna_ecdh_set_ctx_params;
static OSSL_FUNC_keyexch_settable_ctx_params_fn luna_ecdh_settable_ctx_params;
static OSSL_FUNC_keyexch_get_ctx_params_fn luna_ecdh_get_ctx_params;
static OSSL_FUNC_keyexch_gettable_ctx_params_fn luna_ecdh_gettable_ctx_params;
...
static
void *luna_ecdh_newctx(void *provctx)
{
    PROV_ECDH_CTX *pectx;
...
static
int luna_ecdh_init(void *vpecdhctx, void *vecdh, const OSSL_PARAM params[])
{
    ...
    return luna_ecdh_set_ctx_params(pecdhctx, params)
           && luna_prov_ec_check_key(pecdhctx->libctx, vecdh, 1);
}

static
int luna_ecdh_match_params(const EC_KEY *priv, const EC_KEY *peer)
{
    int ret;
    BN_CTX *ctx = NULL;
    ...
    ret = group_priv != NULL
          && group_peer != NULL
          && luna_prov_EC_GROUP_cmp(group_priv, group_peer, ctx) == 0;
    if (!ret)
...
static
int luna_ecdh_set_peer(void *vpecdhctx, void *vecdh)
{
    PROV_ECDH_CTX *pecdhctx = (PROV_ECDH_CTX *)vpecdhctx;
    ...
    if (!luna_prov_is_running()
            || pecdhctx == NULL
            || vecdh == NULL
            || !luna_ecdh_match_params(pecdhctx->k, vecdh)
            || !luna_prov_ec_check_key(pecdhctx->libctx, vecdh, 1)
            || !EC_KEY_up_ref(vecdh))
        return 0;
...
static
void luna_ecdh_freectx(void *vpecdhctx)
{
    PROV_ECDH_CTX *pecdhctx = (PROV_ECDH_CTX *)vpecdhctx;
...
static
void *luna_ecdh_dupctx(void *vpecdhctx)
{
 ...
 err:
    luna_ecdh_freectx(dstctx);
    return NULL;
}

static
int luna_ecdh_set_ctx_params(void *vpecdhctx, const OSSL_PARAM params[])
{
...
static const OSSL_PARAM luna_known_settable_ctx_params[] = {
...
static
const OSSL_PARAM *luna_ecdh_settable_ctx_params(ossl_unused void *vpecdhctx,
                                           ossl_unused void *provctx)
{
    return luna_known_settable_ctx_params;
}

static
int luna_ecdh_get_ctx_params(void *vpecdhctx, OSSL_PARAM params[])
{
    PROV_ECDH_CTX *pectx = (PROV_ECDH_CTX *)vpecdhctx;
    OSSL_PARAM *p;
...
static const OSSL_PARAM luna_known_gettable_ctx_params[] = {
    OSSL_PARAM_int(OSSL_EXCHANGE_PARAM_EC_ECDH_COFACTOR_MODE, NULL),
    OSSL_PARAM_utf8_string(OSSL_EXCHANGE_PARAM_KDF_TYPE, NULL, 0),
    OSSL_PARAM_utf8_string(OSSL_EXCHANGE_PARAM_KDF_DIGEST, NULL, 0),
    OSSL_PARAM_size_t(OSSL_EXCHANGE_PARAM_KDF_OUTLEN, NULL),
...
static
const OSSL_PARAM *luna_ecdh_gettable_ctx_params(ossl_unused void *vpecdhctx,
                                           ossl_unused void *provctx)
{
    return luna_known_gettable_ctx_params;
}

static ossl_inline
size_t luna_ecdh_size(const EC_KEY *k)
{
    size_t degree = 0;
...
static /*ossl_inline*/
int luna_ecdh_plain_derive(void *vpecdhctx, unsigned char *secret,
                      size_t *psecretlen, size_t outlen)
{
    ...
    ecdhsize = luna_ecdh_size(pecdhctx->k);
    if (secret == NULL) {
        *psecretlen = ecdhsize;
        return 1;
    }
    ...
    retlen = luna_prov_ECDH_compute_key_ex(secret, size, pecdhctx->peerk, privk, NULL);
...
#if 0 /* FIXME:luna? */
static /*ossl_inline*/
int luna_ecdh_X9_63_kdf_derive(void *vpecdhctx, unsigned char *secret,
                          size_t *psecretlen, size_t outlen)
{
    PROV_ECDH_CTX *pecdhctx = (PROV_ECDH_CTX *)vpecdhctx;
    ...
    if (!luna_ecdh_plain_derive(vpecdhctx, NULL, &stmplen, 0))
        return 0;
    if ((stmp = OPENSSL_secure_malloc(stmplen)) == NULL) {
        ERR_raise(ERR_LIB_PROV, ERR_R_MALLOC_FAILURE);
        return 0;
    }
    if (!luna_ecdh_plain_derive(vpecdhctx, stmp, &stmplen, stmplen))
        goto err;

    /* Do KDF stuff */
    if (!luna_prov_ecdh_kdf_X9_63(secret, pecdhctx->kdf_outlen,
                             stmp, stmplen,
                             pecdhctx->kdf_ukm,
                             pecdhctx->kdf_ukmlen,
                             pecdhctx->kdf_md,
...
static
int luna_ecdh_derive(void *vpecdhctx, unsigned char *secret,
                size_t *psecretlen, size_t outlen)
{
    PROV_ECDH_CTX *pecdhctx = (PROV_ECDH_CTX *)vpecdhctx;
    ...
    switch (pecdhctx->kdf_type) {
        case PROV_ECDH_KDF_NONE:
            return luna_ecdh_plain_derive(vpecdhctx, secret, psecretlen, outlen);
#if 0
        case PROV_ECDH_KDF_X9_63:
            return luna_ecdh_X9_63_kdf_derive(vpecdhctx, secret, psecretlen, outlen);
#endif
        default:
            break;
    }
...
const OSSL_DISPATCH luna_ecdh_keyexch_functions[] = {
    { OSSL_FUNC_KEYEXCH_NEWCTX, (void (*)(void))luna_ecdh_newctx },
    { OSSL_FUNC_KEYEXCH_INIT, (void (*)(void))luna_ecdh_init },
    { OSSL_FUNC_KEYEXCH_DERIVE, (void (*)(void))luna_ecdh_derive },
    { OSSL_FUNC_KEYEXCH_SET_PEER, (void (*)(void))luna_ecdh_set_peer },
    { OSSL_FUNC_KEYEXCH_FREECTX, (void (*)(void))luna_ecdh_freectx },
    { OSSL_FUNC_KEYEXCH_DUPCTX, (void (*)(void))luna_ecdh_dupctx },
    { OSSL_FUNC_KEYEXCH_SET_CTX_PARAMS, (void (*)(void))luna_ecdh_set_ctx_params },
    { OSSL_FUNC_KEYEXCH_SETTABLE_CTX_PARAMS, (void (*)(void))luna_ecdh_settable_ctx_params },
    { OSSL_FUNC_KEYEXCH_GET_CTX_PARAMS, (void (*)(void))luna_ecdh_get_ctx_params },
    { OSSL_FUNC_KEYEXCH_GETTABLE_CTX_PARAMS, (void (*)(void))luna_ecdh_gettable_ctx_params },
    { 0, NULL }
};
