                        ...
                        size_t sigsize, const unsigned char *tbs, size_t tbslen)
{
    ...
    if (is_composite) {
        ...
        case 1:
            ...
            break;
#if 0 /* coverity:deadcode for now */
        default:
            ERR_raise(ERR_LIB_USER, ERR_R_FATAL);
            CompositeSignature_free(compsig);
            goto endsign;
                          ...
                          size_t tbslen)
{
    ...
    oqs_key = oqsxkey->oqsx_provider_ctx.oqsx_qs_ctx.sig;
    //evpkey = oqsxkey->classical_pkey;
    is_hybrid = (oqsxkey->keytype == KEY_TYPE_HYB_SIG);
    is_composite = (oqsxkey->keytype == KEY_TYPE_CMP_SIG);
    ...
    if (is_hybrid) {
        ...
        if (actual_classical_sig_len < 1 || actual_classical_sig_len > OQS_PROV_MAX_BUFFER) {
    ...
    if (is_composite) {
        ...
        for (i = 0; i < oqsxkey->numkeys; i++) {
            ...
            if (get_oqsname_fromtls(name)) {
                int rctmp = 0;
                if (LUNA_OQS_QUERY_SIG_verify(oqsxkey->lunakeyctx) == LUNA_OQS_OK) {
                    rctmp = LUNA_OQS_SIG_verify_ndx(oqsxkey->lunakeyctx,
                                                final_tbs, final_tbslen, buf,
                                                buf_len, i);
                } else {
                    rctmp = OQS_SIG_verify(oqs_key,
                                           final_tbs, final_tbslen, buf,
        ...
        int rctmp = 0;
        if (LUNA_OQS_QUERY_SIG_verify(oqsxkey->lunakeyctx) == LUNA_OQS_OK) {
            rctmp = LUNA_OQS_SIG_verify_ndx(oqsxkey->lunakeyctx, tbs, tbslen, sig + index,
            ...
            rctmp = OQS_SIG_verify(oqs_key, tbs, tbslen, sig + index,
                                       siglen - classical_sig_len,
                                     ...
                                     const unsigned char *data, size_t datalen)
{
    ...
    if (poqs_sigctx == NULL)
        return 0;
    if (datalen > OQS_PROV_MAX_BUFFER)
        return 0;
    ...
    if (poqs_sigctx->mdctx)
        ...
        if (poqs_sigctx->mdsize > OQS_PROV_MAX_ACCUMULATED)
            return 0;
        if ((poqs_sigctx->mdsize + datalen) > OQS_PROV_MAX_ACCUMULATED)
            return 0;
