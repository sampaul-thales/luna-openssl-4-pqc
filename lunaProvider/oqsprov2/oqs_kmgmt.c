...
static int oqsx_get_hybrid_params(OQSX_KEY *key, OSSL_PARAM params[])
{
    ...
    if (key->comp_pubkey != NULL && key->pubkey != NULL) {
        ...
        if (classical_pubkey_len < 1 || classical_pubkey_len > OQS_PROV_MAX_BUFFER) {
            ERR_raise(ERR_LIB_PROV, OQSPROV_R_INVALID_ENCODING);
            return -1;
        }
    }
    ...
    if (key->comp_pubkey != NULL && key->comp_pubkey[1] != NULL) {
        pq_pubkey = key->comp_pubkey[1];
        pq_pubkey_len = key->pubkeylen - classical_pubkey_len - SIZE_OF_UINT32;
    }
    if (key->comp_privkey != NULL && key->comp_privkey[1] != NULL) {
