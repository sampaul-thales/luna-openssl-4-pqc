...
static int oqsx_spki_pub_to_der(const void *vxkey, unsigned char **pder)
{
    ...
    if (oqsxkey->keytype != KEY_TYPE_CMP_SIG) {
        ...
        if (oqsxkey->numkeys < 1 || oqsxkey->numkeys > OQS_PROV_MAX_KEYS)
            return -1;
        ...
        for (i = 0; i < oqsxkey->numkeys; i++) {
            ...
            if (oqsxkey->pubkeylen_cmp[i] < 1 || oqsxkey->pubkeylen_cmp[i] > OQS_PROV_MAX_BUFFER) {
                buf = NULL;
            } else {
                buflen = (int)oqsxkey->pubkeylen_cmp[i];
            ...
            if (buf == NULL) {
                ...
                rc_continue = sk_ASN1_TYPE_push(sk, aType[i]);
            }
...
static int oqsx_pki_priv_to_der(const void *vxkey, unsigned char **pder)
{
    ...
    int buflen = 0, privkeylen, tmplen;
    ASN1_OCTET_STRING oct;
    int keybloblen;
    STACK_OF(ASN1_TYPE) *sk = NULL;
    char *name;
    ...
    if (oqsxkey->keytype != KEY_TYPE_CMP_SIG) {
        ...
        if (privkeylen < 1 || privkeylen > OQS_PROV_MAX_BUFFER) {
        ...
        if (oqsxkey->numkeys > 1) { // hybrid
            ...
            /* coverity: check for overflowed value */
            if (actualprivkeylen < 1 || actualprivkeylen > OQS_PROV_MAX_BUFFER) {
                ERR_raise(ERR_LIB_USER, OQSPROV_R_INVALID_ENCODING);
                return 0;
            }
...
#    endif
        ...
        tmplen = oqsx_key_get_oqs_public_key_len(oqsxkey);
        /* coverity: check for overflowed value */
        if (tmplen < 1 || tmplen > OQS_PROV_MAX_BUFFER || privkeylen < 1 || privkeylen > OQS_PROV_MAX_BUFFER)
            return -1;

        buflen = privkeylen + tmplen;
        buf = OPENSSL_secure_malloc(buflen);
        if (buf == NULL) {
            ERR_raise(ERR_LIB_USER, ERR_R_MALLOC_FAILURE);
            return -1;
        ...
        for (i = 0; i < oqsxkey->numkeys; i++) {
            ...
            if ((name = get_cmpname(OBJ_sn2nid(oqsxkey->tls_name), i))
                ...
                OPENSSL_free(temp);
                OPENSSL_free(templen);
                PKCS8_PRIV_KEY_INFO_free(p8inf_internal); /* coverity: buf freed here */
                return -1;
            }
            ...
            if (get_oqsname_fromtls(name) == 0) {
                ...
                if (nid == EVP_PKEY_RSA) { // get the RSA real key size
                    ...
                    /* coverity: check for overflowed value */
                    if (buflen >=1 && buflen <= OQS_PROV_MAX_BUFFER)
                        buflen += 4;
                    if (buflen < 1 || buflen > OQS_PROV_MAX_BUFFER
        ...
        if (okey->privkey) {
            if (okey->keytype == KEY_TYPE_CMP_SIG) {
                ...
                for (i = 0; i < okey->numkeys; i++) {
                    ...
                    snprintf(label, sizeof(label), "%s key material:", name);
                    ...
                    if (get_oqsname_fromtls(name) == 0 // classical key
                        ...
                        /* coverity: check for overflowed value */
                        if (okey->privkeylen_cmp[i] > OQS_PROV_MAX_BUFFER) {
                            OPENSSL_free(name);
                            return 0;
                        }
                        privlen = okey->privkeylen_cmp[i];
                ...
                if (okey->numkeys > 1) { // hybrid key
                    ...
                    snprintf(classic_label, sizeof(classic_label), "%s key material:",
                            OBJ_nid2sn(okey->evp_info->nid));
                    DECODE_UINT32(classic_key_len, okey->privkey);
                    /* coverity: check for overflowed value */
                    if (classic_key_len < 1 || classic_key_len > OQS_PROV_MAX_BUFFER) {
                        ERR_raise(ERR_LIB_USER, OQSPROV_R_INVALID_ENCODING);
                        return 0;
                    }
                    if (okey->privkeylen < (classic_key_len + SIZE_OF_UINT32)) {
                        ERR_raise(ERR_LIB_USER, OQSPROV_R_INVALID_ENCODING);
                        return 0;
                    }
        ...
        if (okey->pubkey) {
            if (okey->keytype == KEY_TYPE_CMP_SIG) {
                ...
                if (okey->numkeys > 1) { // hybrid key
                    ...
                    if (okey->pubkeylen < (classic_key_len + SIZE_OF_UINT32)) {
                        ERR_raise(ERR_LIB_USER, OQSPROV_R_INVALID_ENCODING);
                        return 0;
                    }
                    snprintf(classic_label, sizeof(classic_label), "%s key material:",
